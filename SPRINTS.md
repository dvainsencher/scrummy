...
#111  idea   pauta-bootstrap: add a third case for migrating an existing hand-rolled backlog (ROADMAP.md/docs/sprints.md/non-numeric docs/roadmap/*.md/TODO.md) — detection, [x]/[>]/[ ] status-mapping table, sprint-mapping with synthesized goals, "full fidelity not a curated subset" framing (opposite of the existing two cases), spec-content porting guidance (reuse legacy files, don't rederive), delegating to pauta-scratchpad-import for any unstructured notes file alongside the structured backlog, and a cleanup/CLAUDE.md-override prompt mirroring #114. Confirmed needed by a full real-world migration dry run (164 issues, 23 sprints, 22 legacy specs) against a copy of a real project (docs/sprints.md item; see #112-#115 for what that dry run surfaced).
#112  idea   CLI/skill batch-import capability — confirmed real pain, not speculative: migrating ~164 items one `add-issue` call at a time (even scripted) took many sequential tool calls; a structured bulk-import path (e.g. `pauta import <file>` reading a JSON/YAML list of {title,status,sprint}) would let a migration skill propose the whole batch once and execute it in one CLI invocation instead of one-per-item.
#113  idea   `docs/roadmap/` namespace-collision fix — confirmed real during the dry run: a project with its own pre-existing `docs/roadmap/<slug>.md` files ends up with those files mixed alongside pauta's `issues.jsonl`/`sprints.json`/`specs/` after `init`, and cleanup is necessarily incremental (only files actually ported into a spec can be quarantined — quarantining un-ported ones is data loss). At minimum, document this lifecycle (port-then-quarantine, never quarantine-then-port) in the future #111 migration case; only add a configurable data path (env var/`.pautarc`) if documentation proves insufficient.
#114  idea   README/CLAUDE.md template: an explicit "adopting pauta where a project already has its own backlog doc" section, with a copy-paste CLAUDE.md override snippet (point the project's own Roadmap section at `pauta show`, explicitly superseding any global roadmap-sync directive or `/roadmap`-style skill) — the exact edit a #111 migration case would also need to propose.
#115  idea   pauta-refine / migration "refine vs. as-is" worked well in a real dry run (caught a true duplicate and a true overlap-needing-merge cleanly) but exposed a gap: nothing in pauta-refine or pauta-scratchpad-import says what to do when the *sprint structure itself* is ambiguous (e.g. inferring which sprint is "active" from project git history rather than explicit data) — document this as an explicit judgment call to surface to the user, not silently infer, in #111's migration case.
```

#103, #105-#108 scheduled below (see `install-skills-polish` and `issue-quality` sprints). #111-#115 came out of a full real-world migration dry run (a disposable sandbox copy of an active project, never the live repo) using pauta-add-issue/-reorganize/-suggest-batches/-scratchpad-import as the executing skills — not yet scheduled into a sprint.
