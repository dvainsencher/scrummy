...
```
#111  idea   pauta-bootstrap: add a third case for migrating an existing hand-rolled backlog (ROADMAP.md/docs/sprints.md/non-numeric docs/roadmap/*.md/TODO.md) — detection, status-mapping, sprint-mapping with synthesized goals, "full fidelity not a curated subset" framing, delegating to pauta-scratchpad-import for any unstructured notes file alongside the structured backlog. Now executed *through* #116 (writes the mapping artifact) rather than ad hoc add-issue calls — this case is the trigger/detection logic that decides "yes, run pauta-migrate", not the execution mechanism itself.
#112  idea   CLI batch-import capability (e.g. `pauta import <file>`, reading a JSON/YAML list of {title,status,sprint}) — confirmed real pain: migrating ~164 items one `add-issue` call at a time (even scripted) took many sequential tool calls. This is the execution backend that #116's *approved* mapping artifact feeds into — one CLI invocation per migration, not one per item.
#113  idea   `docs/roadmap/` namespace-collision fix: rename the whole legacy directory out of the way (e.g. `git mv docs/roadmap docs/roadmap-legacy`) *before* `pauta init`, so pauta's `docs/roadmap/` is clean from minute one and legacy content is ported from `docs/roadmap-legacy/` at leisure — no per-file quarantine bookkeeping, no mixed-ownership window. Document this as the required first step of #111's migration case.
...
#115  idea   Surface sprint-structure ambiguity (e.g. which sprint is "active", inferred from git history rather than explicit data) as an explicit question in the mapping artifact's flags, not a silent inference #116 makes on its own.
#116  idea   New pauta-migrate skill/mode — mechanical migration only. Reads the source backlog, drafts a mapping table (source ref / proposed title / status / sprint / spec-action / flags — e.g. "possible duplicate of #42", "thin note, needs clarification") and **writes it to a durable file in the project** (e.g. `docs/roadmap/_migration-plan.md`) for the user to read and edit directly — the file *is* the proposal, not a chat narration that scrolls away. Executes against the pauta CLI (via #112) only after the file is explicitly approved, and files exactly what the (possibly user-edited) table says — no silent merging, skipping, or duplicate-resolution. Those stay flagged, not resolved, here; resolving them is #118's job.
