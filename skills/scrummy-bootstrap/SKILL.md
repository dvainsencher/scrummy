...
0. **Check for a legacy backlog doc before anything else** — `ROADMAP.md`,
   ...
   `issues.jsonl`/`sprints.json`/`specs/`, `TODO.md`, or similar. A root
   `ROADMAP.md` that opens with `_Generated by scrummy` is scrummy's own
   derived view (written by `npx scrummy roadmap` / any mutating command) — that's
   not a legacy doc, skip it. If you find a *different* one, stop following
   these steps and follow `scrummy-migrate`'s `SKILL.md` instead —
