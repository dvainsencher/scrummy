...
1. Run `npx scrummy show --json` to read the backlog (issues with no sprint) and the
...
5. Once the user confirms (or edits the proposal), execute via `scrummy` commands
   only:
   - `npx scrummy create-sprint <name> --goal "..."` for new sprints
   - `npx scrummy move <id> <sprint-name>` to assign each issue
   - `npx scrummy set-position <name> <n>` if the user specifies an order
