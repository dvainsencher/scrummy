---
name: scrummy-add-issue
description: This skill should be used when, during or after a feature/design discussion in a project that uses scrummy, the user wants to capture the idea as a backlog issue — phrases like "add this to the backlog", "track this", "let's add an issue for X", "slot this into a sprint", or "add this and slot it".
---
...
1. Run `npx scrummy show --json` to see the current backlog and sprints (names, goals,
...
3. Before filing, run `scrummy-refine`'s definition-clarity check on the candidate
   title (there's no spec yet at this point, so only the clarity check applies —
   consistency/spec-quality are for `scrummy-refine` to apply once the issue exists).
   ...
   - **Filing directly at `--status ready`** is a bigger commitment than the
     `idea` default — run `scrummy-refine`'s *full* check instead (clarity +
     consistency + spec quality), not just clarity, before calling `add-issue`.
4. Call `npx scrummy add-issue "<title>" [--status idea|ready] [--sprint <name>]`. It prints
   the new issue's id.
