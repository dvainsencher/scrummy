---
name: scrummy-migrate
description: This skill executes a mechanical migration of an existing hand-rolled backlog (ROADMAP.md, docs/sprints.md, TODO.md, or similar) into scrummy. It is normally reached through scrummy-bootstrap's migration-detection case, not invoked directly — phrases like "migrate my old backlog into scrummy" or "port this ROADMAP.md" should be redirected to scrummy-bootstrap first so detection runs.
---
...
`scrummy-refine`'s job, run separately and explicitly — never bundled into this skill.

Reached through `scrummy-bootstrap`'s migration case, which has already identified the
...
1. **Rename the legacy directory out of the way first**, if any source content
   ...
   required before `npx scrummy init` — `npx scrummy init` refuses to run if `docs/roadmap/`
   contains anything other than scrummy's own files (see #113), and you need scrummy's
   `docs/roadmap/` clean from minute one rather than mixing ownership.
2. Run `npx scrummy init` (if not already done) so scrummy's own `docs/roadmap/` exists,
   then `npx scrummy show --json` to confirm it's empty — this skill is for a first
...
3. Read every source file in full (after the rename, legacy `docs/roadmap/*`
   ...
   `scrummy-scratchpad-import` does — one idea per row, don't split one thought
...
7. Execute exactly what the (possibly edited) file says, in this order:
   - `npx scrummy create-sprint <name> --goal "<synthesized goal>"` for each sprint
     named in the table that doesn't already exist.
   - Build a JSON array of `{title, status, sprint}` from every issue row and run
     `npx scrummy import <file>` once — don't call `add-issue` per row.
